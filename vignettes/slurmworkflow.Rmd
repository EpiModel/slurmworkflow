...
only have one value for each (1-6, 2-7, 3-8, 4-9 and 5-10). Similar to `Map`, the
`MoreArgs` argument defines the values to be shared across replication.

A new argument `max_array_size` as been set to 2. This means that if we had
more than 2 values to iterate over, the step template would split the job in
several submission of at most 2 array jobs. So in this our case, we have
submissions of array size 2, 2, and 1. This prevents SLURM from refusing the
submission when too many jobs at once, there is usually a limit of 10000. (With
EpiModel we already had cases where 5000 array jobs were submitted. This
template simply submit them in chunks of 500)
